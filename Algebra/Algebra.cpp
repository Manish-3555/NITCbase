...
int Algebra::insert(char relName[ATTR_SIZE], int nAttrs, char record[][ATTR_SIZE]){
    ...
    if (ret != SUCCESS)
        return ret;
    /* if relCatEntry.numAttrs != numberOfAttributes in relatio5n,
...
/*RecBuffer relCatBuffer(RELCAT_BLOCK);
    ...
    for (int i = 2; i < relCatHeader.numEntries; i++)
    {
        ...
        printf("Relation: %s, count:%d\n", relCatRecord[RELCAT_REL_NAME_INDEX].sVal, count);
    }*/

    /*char relName[] = "S11_Students"; 
    char attrName[] = "name";
    ...
    if (relId != E_RELNOTOPEN) {
        AttrCatEntry attrCatEntry;
        int r = AttrCacheTable::getAttrCatEntry(relId, attrName, &attrCatEntry);

        if (r == SUCCESS) {
            int rootBlock = attrCatEntry.rootBlock;

            if (rootBlock != -1) {
                BPlusTree::bPlusprint(rootBlock);
            }
        }
    }*/