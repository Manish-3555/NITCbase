...
void BPlusTree::bPlusPrintLevelOrder(int rootBlockNum) {
        ...
        for (int i = 0; i < levelSize; i++) {
            ...
            if (type == IND_INTERNAL) {
                ...
                for (int j = 0; j < head.numEntries; j++) {
                    indIntBlk.getEntry(&entry, j);
                    if (entry.rChild != -1)
                        q.push(entry.rChild);
                }

            } else if (type == IND_LEAF) {
                printf("[Leaf: %d] ", blockNum);

            } else {
                printf("[Invalid: %d] ", blockNum);
            }
        }
        printf("\n");
        level++;
...
int BPlusTree::bPlusprint(int rootBlockNum) {
    /* if rootBlockNum lies outside the valid range [0,DISK_BLOCKS-1]*/
    if (rootBlockNum < 0 || rootBlockNum >= DISK_BLOCKS) {
        return E_OUTOFBOUND;
    }
    ...
    if (type == IND_LEAF) {
        ...
        // load the header of the block using BlockBuffer::getHeader().
        printf("[Internal: %d] ", rootBlockNum);
        HeadInfo head;
        indIntBlk.getHeader(&head);
        ...
        indIntBlk.getEntry(&entry, 0);
        if (entry.lChild != -1)
        bPlusprint(entry.lChild);
        for (int i = 0; i < head.numEntries; i++) {
            indIntBlk.getEntry(&entry, i);
            if (entry.rChild != -1)
            bPlusprint(entry.rChild);
        }
